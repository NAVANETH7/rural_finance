...
exports.handleCopilotQuery = void 0;
...
const handleCopilotQuery = async (req, res) => {
    try {
        ...
        let recsCount = 0;
        if (businessId) {
            const biz = await models_1.Business.findById(businessId);
            if (biz) {
                businessName = biz.name;
                revenue = biz.monthlyIncome;
                profit = biz.monthlyIncome - biz.monthlyExpenses;
                // Fetch outstanding loans
                const activeLoans = await models_1.Loan.find({ business: businessId, status: 'Active' });
                ...
                const pendingRecs = await models_1.Recommendation.find({ business: businessId, status: 'pending' });
        ...
        if (cleanMsg.includes('loan') || cleanMsg.includes('borrow') || cleanMsg.includes('eligible')) {
            ...
            responseText = `Your current AI Credit Risk is rated at **${riskScore}% (${riskSeverity} Risk)**. ${riskSeverity === 'High'
...
exports.handleCopilotQuery = handleCopilotQuery;
