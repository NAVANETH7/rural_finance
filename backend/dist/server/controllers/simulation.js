...
exports.simulateScenario = void 0;
const models_1 = require("../models");
const simulateScenario = async (req, res) => {
    try {
        const { businessId, revenueChangePercent = 0, expenseChangePercent = 0, newLoanAmount = 0, oneTimeExpense = 0, hvacDimmingPercent = 0, solarUpgradeKw = 0 } = req.body;
        let baselineIncome = 45000;
        let baselineExpenses = 28000;
        let businessName = 'Rural Store Demo';
        if (businessId) {
            const b = await models_1.Business.findById(businessId);
            if (b) {
