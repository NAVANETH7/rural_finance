...
import { Prediction, Transaction } from '../models';
import { redisClient } from '../config/redis';
import axios from 'axios';

const FASTAPI_URL = process.env.FASTAPI_URL || 'http://localhost:8000';

export const getCashflowPrediction = async (req: AuthenticatedRequest, res: Response) => {
  try {
    ...
    try {
      ...
      const response = await axios.post(`${FASTAPI_URL}/predict/cashflow`, payload, { timeout: 3000 });
    ...
    await redisClient.setEx(cacheKey, 3600, JSON.stringify(forecastResult));
