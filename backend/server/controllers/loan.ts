...
import { Loan, Business, Notification } from '../models';
...
export const getLoan = async (req: AuthenticatedRequest, res: Response) => {
  try {
...
export const listLoans = async (req: AuthenticatedRequest, res: Response) => {
  try {
    const filter: any = {};
    ...
    const loans = await Loan.find(filter).populate('business').populate('user', 'email profile');
    res.json(loans);
  } catch (error: any) {
    res.status(500).json({ message: error.message });
  }
...
export const getLoanEligibility = async (req: AuthenticatedRequest, res: Response) => {
  try {
    ...
    const breakdown: string[] = ['Baseline stability: 50 points'];
